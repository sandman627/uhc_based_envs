...
def test_embodiedpose_env():
    ...
    cfg.output = os.path.join(prox_path, "renderings/sceneplus", f"{cfg.id}")
...
def test_HumanoidKinEnvRes():
    ...
    # data_loader
    data_loader = get_data_loader(cfg=cfg)
...
def get_env(cfg, data_loader, policy_net, global_start_fr=0):
    """load CC model"""
...
def get_policy_net(cfg, data_loader, device, dtype, global_start_fr, mode):
    data_sample = data_loader.sample_seq(fr_num=20, fr_start=global_start_fr)
